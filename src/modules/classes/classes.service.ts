// REMOVE DUPLICATED LEGACY BLOCK ABOVE; USE THE MAIN SERVICE BELOW
...
export class ClassesService {
  ...
  /**
   * Chuẩn hóa điểm số từ payload (có thể là string hoặc number) về number an toàn.
   */
  private normalizeScoreValue(value: any): number {
  ...
  private buildClassInclude(options?: {
    includeParticipants?: boolean;
    includeExamScores?: boolean;
    includeParticipantContact?: boolean;
  }) {
    const include: Prisma.classesInclude = {
      class_group: {
        select: {
          class_group_id: true,
          name: true,
  ...
  // ========================================================
  // 🏫 CLASS MANAGEMENT - SYSTEM ADMIN & ACADEMIC MANAGER
  // ========================================================

  async create(createClassDto: CreateClassDto, createdBy: number) {
    ...
    if (!classGroup) {
      ...
      throw new BadRequestException('Nhóm lớp không tồn tại');
    }

    const startTime = normalizeVietnamTimestamp(createClassDto.start_time);
    if (!startTime) {
      throw new BadRequestException('start_time không hợp lệ');
    }
    const endTime = normalizeVietnamTimestamp(createClassDto.end_time);
    
    // Chỉ set attendance time nếu thực sự có giá trị được truyền vào
    const attendanceStartTime = createClassDto.attendance_start_time !== undefined
      ? normalizeVietnamTimestamp(createClassDto.attendance_start_time)
      : undefined;
    const attendanceEndTime = createClassDto.attendance_end_time !== undefined
      ? normalizeVietnamTimestamp(createClassDto.attendance_end_time)
      : undefined;
    
    const timestampNow = getVietnamCurrentDate();
    ...
    try {
      ...
      const classData = await this.prisma.classes.create({
        data: {
          ...
          ...(typeof (createClassDto as any).status !== 'undefined'
            ? { status: (createClassDto as any).status }
            : {}),
      ...
      await this.logActivity(createdBy, 'class.create', {
        ...
        payload: this.sanitizePayload(createClassDto as Record<string, any>),
      });
  ...
  async update(id: number, updateClassDto: UpdateClassDto, updatedBy?: number) {
    try {
      const {
        class_group_id,
        start_time,
      ...
      if (start_time !== undefined) {
        const normalized = normalizeVietnamTimestamp(start_time);
        if (!normalized) {
          throw new BadRequestException('start_time không hợp lệ');
        }
      ...
      if (end_time !== undefined) {
        data.end_time = normalizeVietnamTimestamp(end_time);
      }
      if (attendance_start_time !== undefined) {
        // Cho phép set về null để xóa attendance time
        data.attendance_start_time = attendance_start_time === null 
          ? null 
          : normalizeVietnamTimestamp(attendance_start_time);
      }
      if (attendance_end_time !== undefined) {
        // Cho phép set về null để xóa attendance time
        data.attendance_end_time = attendance_end_time === null 
          ? null 
          : normalizeVietnamTimestamp(attendance_end_time);
      }

      data.updated_at = getVietnamCurrentDate();
      ...
      if (updatedBy) {
        await this.logActivity(updatedBy, 'class.update', {
          class_id: id,
          title: classData.title,
          changes: this.sanitizePayload(updateClassDto as Record<string, any>),
        });
      }
  ...
  // ========================================================
  // 👥 PARTICIPANT MANAGEMENT - ACADEMIC MANAGER & TEACHER
  // ========================================================

  async addParticipant(classId: number, userId: number, addedBy?: number) {
    try {
      const timestampNow = getVietnamCurrentDate();
      ...
      if (error.code === 'P2002') {
        throw new BadRequestException('User already enrolled in this class');
      }
      throw error;
    }
  }

  async removeParticipant(classId: number, userId: number, removedBy?: number) {
    try {
  ...
  async updateAttendance(
    classId: number,
    userId: number,
    payload: { attendance_status?: string; feedback?: string },
    updatedBy?: number,
  ) {
    try {
      // Verify user has valid registration status
      const classData = await this.prisma.classes.findUnique({
      ...
      if (!registration) {
        throw new BadRequestException('Student must have valid registration status (paid, active, or completed) to mark attendance');
      }
  ...
  async getClassParticipants(classId: number) {
  ...
  // ========================================================
  // 📝 EXAM SCORE MANAGEMENT - TEACHER & ACADEMIC MANAGER
  // ========================================================

  async addExamScore(classId: number, userId: number, scoreData: any, gradedBy: number) {
    try {
      ...
      if (error.code === 'P2002') {
        throw new BadRequestException('Exam score already exists for this user');
      }
      throw error;
    }
  }

  async updateExamScore(classId: number, userId: number, scoreData: any, updatedBy?: number) {
    try {
      const listening = this.normalizeScoreValue(scoreData.listening_score);
      const speaking = this.normalizeScoreValue(scoreData.speaking_score);
      const reading = this.normalizeScoreValue(scoreData.reading_score);
  ...
  async getClassExamScores(classId: number) {
  ...
  async getTeacherClasses(teacherId: number, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  async updateClassStatus(classId: number, status: 'scheduled' | 'live' | 'completed' | 'cancelled', updatedBy?: number) {
    try {
      const classData = await this.prisma.classes.update({
        where: { class_id: classId },
  ...
  async getUpcomingClasses(days: number = 7) {
  ...
  async getClassStats() {
    const [
      ...
      this.prisma.classes.count({ 
        where: { 
          start_time: { gt: getVietnamCurrentDate() },
          status: 'scheduled',
        },
      }),
  ...
  async updateAttendanceStatus(classId: number, userId: number, status: string, updatedBy?: number) {
    return this.updateAttendance(classId, userId, { attendance_status: status }, updatedBy);
  }

  async recordExamScore(classId: number, userId: number, scoreData: any, gradedBy?: number) {
    // Verify class exists and is a valid offline exam session
    const classData = await this.prisma.classes.findUnique({
      where: { class_id: classId },
      select: {
        class_type: true,
    ...
    if (classData.class_type !== 'offline' || !classData.exam_type) {
      throw new BadRequestException('Chỉ được chấm điểm cho các buổi thi offline (có exam_type)');
    }
    ...
    if (!registration) {
      throw new BadRequestException('Student must have valid registration status (paid, active, or completed) to be graded');
    }
  ...
  // === Added helpers for controllers ===
  async isTeacherOfClass(teacherId: number, classId: number): Promise<boolean> {
  ...
  async listParticipants(classId: number) {
    // Get class to get course_id
    const classData = await this.prisma.classes.findUnique({
      where: { class_id: classId },
      select: {
        class_group: {
    ...
    // Get valid registrations (paid, active, completed)
    ...
    // Filter participants to only include valid registered students
  ...
  async listExamScores(classId: number) {
    // Get class to get course_id
    const classData = await this.prisma.classes.findUnique({
      where: { class_id: classId },
      select: {
    ...
    // Filter exam scores to only include valid registered students
  ...
  async gradeStudent(classId: number, userId: number, payload: any, gradedBy?: number) {
    return this.recordExamScore(classId, userId, payload, gradedBy);
  }

  /**
   * Lấy lịch giảng dạy theo tuần cho giáo viên
   * @param teacherId ID của giáo viên
   * @param weekStart Ngày bắt đầu tuần (YYYY-MM-DD)
   * @returns Lịch giảng dạy được nhóm theo ngày và buổi
   */
  async getTeachingSchedule(teacherId: number, weekStart: string) {
    try {
      ...
      if (!weekStart || !/^\d{4}-\d{2}-\d{2}$/.test(weekStart)) {
        ...
        throw new BadRequestException(`Invalid weekStart format. Expected YYYY-MM-DD, got: ${weekStart}`);
      }
      
      // Parse weekStart và tính toán ngày kết thúc tuần
      ...
      if (!year || !month || !day || month < 1 || month > 12 || day < 1 || day > 31) {
        ...
        throw new BadRequestException(`Invalid date: ${weekStart}`);
      }
      ...
      // Đơn giản: Chỉ lấy classes mà teacher_id = user_id
    const classes = await this.prisma.classes.findMany({
      where: {
          teacher_id: teacherId,
        start_time: {
    ...
    // Nhóm classes theo ngày và buổi
    ...
    // Phân loại classes vào các buổi
    classes.forEach((classItem) => {
      ...
      // Xác định buổi dựa trên giờ bắt đầu
      ...
      if (schedule[dateKey]) {
        // Xử lý trường hợp class không có class_group
        ...
        if (classItem.class_group) {
          ...
          // Nếu không có class_group trong include, tạo dữ liệu mặc định
          classGroupData = {
      ...
      // Tìm tuần gần nhất có lịch học (nếu không có lịch trong tuần này)
      let nearestWeekWithSchedule: string | null = null;
      if (classes.length === 0) {
        this.logger.warn('⚠️ No classes found in current week, searching for nearest week...');
        // Tìm class gần nhất trong tương lai
        ...
        // Tìm class gần nhất trong quá khứ
        ...
        // Ưu tiên class trong tương lai, nếu không có thì lấy class trong quá khứ
    ...
    return {
      ...
      schedule,
        nearestWeekWithSchedule, // Tuần gần nhất có lịch học
